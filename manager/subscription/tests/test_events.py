...
class TestTelemetrySubscription:
    ...
    # Tests for 1 event stream
    @pytest.mark.asyncio
    @pytest.mark.django_db
    async def test_join_event_stream(self):
        """ Test that clients can join a event stream """
        # Arrange
    ...
    @pytest.mark.asyncio
    @pytest.mark.django_db
    async def test_leave_event_stream(self):
        """ Test that clients can leave a event stream """
        # Arrange
        communicator = WebsocketCommunicator(application, self.url)
        connected, subprotocol = await communicator.connect()
        await communicator.send_json_to(subscription_msg)
    ...
    @pytest.mark.asyncio
    @pytest.mark.django_db
    async def test_receive_event_stream(self):
        """ Test that clients can receive messages from a event streams """
        # Arrange
        communicator = WebsocketCommunicator(application, self.url)
        connected, subprotocol = await communicator.connect()
        await communicator.send_json_to(subscription_msg)
    ...
    @pytest.mark.asyncio
    @pytest.mark.django_db
    async def test_not_receive_event_from_left_stream(self):
        """ Test that clients cannot receive a event of a stream they have left """
        # Arrange
        communicator = WebsocketCommunicator(application, self.url)
        connected, subprotocol = await communicator.connect()
        await communicator.send_json_to(subscription_msg)
    ...
    # Tests for all event stream
    @pytest.mark.asyncio
    @pytest.mark.django_db
    async def test_join_all_telemetries(self):
        """ Test that clients can join all the event streams """
        # Arrange
        communicator = WebsocketCommunicator(application, self.url)
        connected, subprotocol = await communicator.connect()
        # Act
    ...
    @pytest.mark.asyncio
    @pytest.mark.django_db
    async def test_leave_all_event_streams(self):
        """ Test that clients can leave all event streams """
        # Arrange
        communicator = WebsocketCommunicator(application, self.url)
        connected, subprotocol = await communicator.connect()
        await communicator.send_json_to(subscription_all_msg)
    ...
    async def test_receive_all_telemetries(self):
        """ Test that clients can receive messages from all the event streams """
        # Arrange
        communicator = WebsocketCommunicator(application, self.url)
        connected, subprotocol = await communicator.connect()
        await communicator.send_json_to(subscription_all_msg)
    ...
    @pytest.mark.asyncio
    @pytest.mark.django_db
    async def test_not_receive_event_after_leaving_all_streams(self):
    ...
    # Tests for 2 event streams
    @pytest.mark.asyncio
    @pytest.mark.django_db
    async def test_join_2_telemetries(self):
        """ Test that clients can join 2 of the event streams """
        # Arrange
        communicator = WebsocketCommunicator(application, self.url)
        connected, subprotocol = await communicator.connect()
        # Act
    ...
    async def test_receive_2_telemetries(self):
        """ Test that clients can receive messages from 2 of the event streams """
        # Arrange
        communicator = WebsocketCommunicator(application, self.url)
        connected, subprotocol = await communicator.connect()
        await communicator.send_json_to(subscription_msg)
    ...
    async def test_receive_telemetries_for_subscribed_streams_only(self):
        """ Test that clients can receive messages only for subscribed event streams """
        # Arrange
        communicator = WebsocketCommunicator(application, self.url)
        connected, subprotocol = await communicator.connect()
        await communicator.send_json_to(subscription_msg)
